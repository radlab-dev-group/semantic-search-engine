...
project_root = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../../../sse_rest_api")
)
...
def prepare_parser():
    parser = argparse.ArgumentParser(
        description="Evaluate RAG quality using LLM as a judge."
    )
    parser.add_argument(
        "-u",
        "--username",
        required=True,
        type=str,
        help="Username to use for requests",
    )
    parser.add_argument(
        "-c", "--collection", required=True, type=str, help="Collection name"
    )
    parser.add_argument(
        "-i",
        "--input-json",
        required=True,
        type=str,
        help="Input JSON file with questions and ground truth",
    )
    parser.add_argument(
        "-o",
        "--output-json",
...
class RAGEvaluator:
    def __init__(
        self, user: OrganisationUser, collection_name: str, eval_model: str
    ):
        self.user = user
        self.collection = RelationalDBController.get_collection(
    ...
    def evaluate_sample(self, question: str, ground_truth: str = None) -> Dict:
        ...
        chat = self.chat_controller.new_chat(
            ...
            options={
                ...
                "use_intelligent_query_rewrite": True,
                "rerank_results": True,
            },
        )
            ...
            response_text, _, state, _ = (
                self.chat_controller.generate_assistant_message_cs_rag(
                    ...
                    options={
                        ...
                        "user_msg_is_rag_question": True,
                        "rerank_results": True,
                        "generative_model": "google/gemini-2.5-flash-lite",
                    },
                    organisation_user=self.user,
        ...
        eval_results = self._llm_judge(
            question, response_text, context, ground_truth
        )
...
"""
        try:
            eval_str, _ = (
                self.gen_controller.gen_model_controller.conversation_with_local_model(
                    history=[],
