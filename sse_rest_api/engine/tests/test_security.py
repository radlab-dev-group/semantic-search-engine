...
class SecurityIDORTestCase(TestCase):
    def setUp(self):
        ...
        self.org_user1 = OrganisationUser.objects.create(
            auth_user=self.user1, organisation=self.org
        )

        self.user2 = User.objects.create_user(username="user2")
        self.org_user2 = OrganisationUser.objects.create(
            auth_user=self.user2, organisation=self.org
        )
        ...
        self.answer1 = UserQueryResponseAnswer.objects.create(
            user_response=self.response1, is_generative=True, answer_options={}
        )

    def test_get_user_response_by_id_owner(self):
        # User 1 should be able to access their own response
        resp = SearchQueryController.get_user_response_by_id(
            self.response1.pk, self.org_user1
        )
    ...
    def test_get_user_response_by_id_non_owner(self):
        ...
        resp = SearchQueryController.get_user_response_by_id(
            self.response1.pk, self.org_user2
        )
        self.assertIsNone(resp)

    def test_get_user_query_response_answer_owner(self):
        # User 1 should be able to access their own answer
        ans = GenerativeModelController.get_user_query_response_answer(
            self.answer1.pk, self.org_user1
        )
    ...
    def test_get_user_query_response_answer_non_owner(self):
        ...
        ans = GenerativeModelController.get_user_query_response_answer(
            self.answer1.pk, self.org_user2
        )
